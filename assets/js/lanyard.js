...
lanyard.onopen = function () {
  lanyard.send(
    JSON.stringify({
      op: 2,
      d: {
        subscribe_to_id: "880471472550871051",
      },
    })
  );
};
...
function update_presence() {
  ...
  if (api.d.listening_to_spotify == true) {
    ...
    spotifyListening.innerHTML = `<a href="https://open.spotify.com/track/${api.d.spotify.track_id}" target="_blank" class="text-black"><i class="fa-brands fa-spotify text-green-500 mr-2"></i>${song} dinliyor... Şu tarafından: ${artist}</a>`;
  } else {
